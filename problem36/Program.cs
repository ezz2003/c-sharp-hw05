...
int AmountInOddPositions(int[] ar)
{
  int amount = 0;
  for (int i = 1; i < ar.Length; i += 2) amount += ar[i];
  return amount;
}

int[] readyArr = MakeArrRnd(23, -99, 100);
Console.WriteLine($"{MakeStrPrintArr(readyArr)} -> {AmountInOddPositions(readyArr)}");